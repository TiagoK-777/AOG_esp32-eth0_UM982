...
void EthernetStart()
{
  // start the Ethernet connection for ESP32 WT32-ETH01:
  Serial.println("Initializing ESP32 Ethernet with static IP address");
  ...
  IPAddress local_ip(Eth_myip[0], Eth_myip[1], Eth_myip[2], Eth_myip[3]);
  IPAddress gateway(Eth_myip[0], Eth_myip[1], Eth_myip[2], 1);
  ...
  Serial.print("\r\nEthernet IP of module: "); Serial.println(ETH.localIP());
